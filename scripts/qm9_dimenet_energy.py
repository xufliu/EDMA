...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="QM9 DimeNet/DimeNet++ EDMA statistics")
    ...
    parser.add_argument("--node-feat-size", default="1.0")
    parser.add_argument("--node-feat-ent", default="0.0005")
    ...
    parser.add_argument(
        "--dimenet",
        choices=("plusplus", "base"),
        default="plusplus",
        help="Use DimeNet++ by default.",
    )
    parser.add_argument(
        "--output",
        default="dimenet_u_energy_final.pkl",
...
def main() -> None:
    ...
    from edma.models.pretrained import load_pretrained_dimenet_qm9

    start_time = time.time()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model, splits = load_pretrained_dimenet_qm9(
        data_path=args.data_path,
        target_attr=args.target_attr,
        device=device,
        use_dimenet_plus_plus=args.dimenet == "plusplus",
    )
    ...
    for epoch, node_feat_size, node_feat_ent in parameter_combinations:
        ...
        explainer = Explainer(
            model=model,
            algorithm=DimeNetEnergyInstanceExplainer(
                epochs=epoch,
                lr=args.lr,
